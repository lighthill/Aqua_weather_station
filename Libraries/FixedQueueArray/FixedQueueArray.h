 ...
 *  FixedQueueArray.h
 *
 *	This is a modified version of the QueueArray arduino library
 *	that prevents resizing of the array and has a fixed size of 64 elements.
 *
 ...
 *  Version 1.0
 *
 *    2014-05-26 anonomous
* 	- made queue size fixed, prevent calling of resize()
 *
...
// init the queue (constructor).
template<typename T>
FixedQueueArray<T>::FixedQueueArray () {
...
// resize the size of the queue.
template<typename T>
void FixedQueueArray<T>::resize (const int s) {
  // defensive issue.
  if (s <= 0)
    exit (F("QUEUE: error due to undesirable size for queue size."));
  ...
  // if there is a memory allocation error.
  if (temp == NULL)
    exit (F("QUEUE: insufficient memory to initialize temporary queue."));
...
// add an item to the queue.
template<typename T>
void FixedQueueArray<T>::enqueue (const T i) {
  // check if the queue is full.
  if (!isFull ())
  {
...
// push an item to the queue.
template<typename T>
void FixedQueueArray<T>::push (const T i) {
  enqueue(i);
}

// remove an item from the queue.
template<typename T>
T FixedQueueArray<T>::dequeue () {
  // check if the queue is empty.
  if (isEmpty ())
...
// pop an item from the queue.
template<typename T>
T FixedQueueArray<T>::pop () {
  return dequeue();
}

// get the front of the queue.
template<typename T>
T FixedQueueArray<T>::front () const {
  // check if the queue is empty.
  if (isEmpty ())
    exit (F("QUEUE: can't get the front item of queue: queue is empty."));
...
// get an item from the queue.
template<typename T>
T FixedQueueArray<T>::peek () const {
  return front();
}

// check if the queue is empty.
template<typename T>
bool FixedQueueArray<T>::isEmpty () const {
  return items == 0;
}

// check if the queue is full.
template<typename T>
bool FixedQueueArray<T>::isFull () const {
  return items == size;
}

// get the number of items in the queue.
template<typename T>
int FixedQueueArray<T>::count () const {
  return items;
}

// set the printer of the queue.
template<typename T>
void FixedQueueArray<T>::setPrinter (Print & p) {
  printer = &p;
}

// exit report method in case of error.
template<typename T>
void FixedQueueArray<T>::exit (const char * m) const {
...
// overloaded exit function for PROGMEM F() macro
template<typename T>
void FixedQueueArray<T>::exit (const __FlashStringHelper * m)  const {
  // print the message if there is a printer.
  if (printer)
    printer->println (m);
...
// led blinking method in case of error.
template<typename T>
void FixedQueueArray<T>::blink () const {
